...
def get(target)
  get_keepalive(target)
end

@alive_conn = Net::HTTP.new('localhost',8080)
@alive_conn.start
def get_keepalive(target)
  uri = URI(target)
  @alive_conn.get(uri.path).body
end

def get_simple(target)
  uri = URI(target)
  Net::HTTP.get(uri)
end
...
@vols.each_pair do |k, v|
  puts "#{k.to_s.rjust(8, ' ')}: #{v/1024}k bytes"
end
